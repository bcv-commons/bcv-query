...
Output columns: strong, keyness, anchor
"""
...
def hebrew_keyness() -> dict[str, float]:
    """H#### keyness: spine OT zipf − modern Hebrew general zipf."""
    nt = sorted(NT_BOOKS)
    ph = ",".join("?" * len(nt))
    ...
    for strong, c, lemma in rows:
        ...
        out[_norm("H", strong)] = round(zb - zg, 2)
    return out


def greek_keyness() -> dict[str, float]:
    """G#### keyness via English carry-over: BSB gloss zipf − general English zipf."""
    # English gloss per Greek Strong's, from strongs_gloss.tsv (lang=en).
    en_gloss: dict[str, str] = {}
    with GLOSS_TSV.open(encoding="utf-8") as fh:
        next(fh, None)
        for line in fh:
            p = line.rstrip("\n").split("\t")
            if len(p) >= 4 and p[3] == "en" and p[0].startswith("G"):
                en_gloss[p[0]] = p[1]
    ...
    for code, gloss in en_gloss.items():
        ...
        if best is not None:
            out[_pad_code(code)] = round(best, 2)  # canonical padded (G26→G0026)
    return out
...
def main() -> None:
    ...
    rows = ([(c, k, "he") for c, k in heb.items()]
            + [(c, k, "en") for c, k in grk.items()])
    rows.sort(key=lambda r: (-r[1], r[0]))

    with OUTPUT.open("w", encoding="utf-8") as fh:
        fh.write("strong\tkeyness\tanchor\n")
        for code, k, anchor in rows:
            fh.write(f"{code}\t{k}\t{anchor}\n")

    print(f"\nWrote {len(rows)} entries to {OUTPUT}", file=sys.stderr)
    print("Most distinctive (top 8):", file=sys.stderr)
    for code, k, anchor in rows[:8]:
        print(f"  {code}\t{k:+.2f}\t{anchor}", file=sys.stderr)
