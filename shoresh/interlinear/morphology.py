"""Grammar/morphology code expansion, ported from example/aleph/data-api/src/morphology.js
(itself ported from study-app's word_details_dialog/dialog_manager.dart)."""
from __future__ import annotations

MORPHOLOGY_CODES: dict[str, str] = {
...
def expand_grammar(grammar: str) -> str:
    """Expands a hyphen-separated grammar code (e.g. "Prep-l" or "V-AIA-3ms") into readable text."""
    parts = grammar.split("-")
    out = ""
    for i, part in enumerate(parts):
        expanded = MORPHOLOGY_CODES.get(part, part)
        if i == 0:
            out += expanded[:1].upper() + expanded[1:]
        elif i == 1:
            out += ": " + expanded
        else:
            out += ", " + expanded
